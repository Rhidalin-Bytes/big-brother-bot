...
class DupenamePlugin(b3.plugin.Plugin):
  ...
  def onLoadConfig(self):
    ...
    try:
        self._checkdupes = self.config.get('settings', 'checkdupes')
    except:
        self._checkdupes = 1
        self.debug('Using default value (%s) for checkdupes', self._checkdupes)
    try:
        self._checkunknown = self.config.get('settings', 'checkunknown')
    except:
        self._checkunknown = 1
  ...
  def check(self):    
    ...
    if self.isEnabled() and (self.console.time() > self._ignoreTill):     
      ...
      for pname,cidlist in d.items():
        if self._checkdupes == 1 and len(cidlist) > 1:
          for cid in cidlist:
            client = self.console.clients.getByCID(cid)
            self._adminPlugin.warnClient(client, 'badname')

        if self._checkunknown == 1 and pname == 'Unknown Soldier':
          for cid in cidlist:
            client = self.console.clients.getByCID(cid)
            self._adminPlugin.warnClient(client, 'badname')
