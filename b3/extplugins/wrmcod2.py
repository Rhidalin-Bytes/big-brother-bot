...
#-------------------------------------------------------------
class Wrmcod2Plugin(b3.plugin.Plugin):
  _adminPlugin = None
...
#**START OF WRM COMMANDS**
  ...
  def cmd_wrmvsclan(self, data, client, cmd=None):
    ...
    var_vsclan1 = self.console.getCvar('wrm_clanTags1')[0]
    if not data:         
         if not var_vsclan1:
               ...
               cmd.sayLoudOrPM(client, '^7Current clan tags: ^1%s' % var_vsclan1)
               return False
    else:
      # check to see what they entered
      if data == 'none':
           self.console.setCvar( 'wrm_clanTags1','')
           self.console.say('^9Clan tags set to : ^1none')
      else:
           self.console.setCvar( 'wrm_clanTags1','%s' % data )
           self.console.say('^9Clan tags set to : ^1%s' % data)
    return True
    
  def cmd_wrmvsclan1(self, data, client, cmd=None):
    """\
    <tags> - Set the clan tags for a clan vs. match (colors excluded)  
    """
    # we need the cvar of the clan tags
  ...
  def cmd_wrmvsclan2(self, data, client, cmd=None):
    """\
    <tags> - Set the clan tags for a clan vs. match (colors excluded)  
    """
    # we need the cvar of the clan tags
    var_vsclan = self.console.getCvar('wrm_clanTags2')[0]
    if not data:         
         if not var_vsclan2:
               # it hasnt been set in the config yet, no problem                
               client.message('^7No clan set yet, use !help wrmvsclan for usage')
               ...
               cmd.sayLoudOrPM(client, '^7Current clan tags: ^1%s' % var_vsclan2)
               return False
    else:
      # check to see what they entered
      if data == 'none':
           self.console.setCvar( 'wrm_clanTags2','')
           self.console.say('^9Clan tags set to : ^1none')
      else:
           self.console.setCvar( 'wrm_clanTags2','%s' % data )
           self.console.say('^9Clan tags set to : ^1%s' % data)
    return True
...
#        cmd.sayLoudOrPM(client,'^1%s (%s) ^9guid: ^1%s ^9country: ^1%s ^9ip: ^1%s ^9lvl: ^1%s' % (sclient.exactName, str(sclient.id), str(sclient.guid), str(country), str(sclient.ip), str(sclient.maxLevel)))
  ...
  def getMods(self):
    return self.console.clients.getClientsByLevel(20,20)

  def cmd_mods(self, data, client, cmd=None):
    """\
    - lists all the online moderators
    """
    self.debug('trying to get mods')
    clist = self.getMods()
    ...
    if len(clist) > 0:
        ...
        cmd.sayLoudOrPM(client, self.getMessage('mods', string.join(nlist, ', ')))
    else:
        self.debug('no moderators found')
        cmd.sayLoudOrPM(client, 'no moderators found')
